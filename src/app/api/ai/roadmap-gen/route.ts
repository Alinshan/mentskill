...
`;
      ...
      try {
        ...
        console.warn("Second JSON repair failed, using raw LLM output", repairErr);
        secondRepairedJSON = secondResponse.content as string; // fallback
      }

      // Parse second response
      finalParsed = await parser.parse(secondRepairedJSON);
      
    } catch (llmError: any) {
      console.warn("[MOCK INTERCEPTOR] Roadmap Groq request bypassed. Delivering Sandbox Demo Sequence:", llmError.message);
      
      finalParsed = {
        roadmapTitle: `${field} ${mode} Sandbox Journey`,
        description: `An optimized test ${timeline} pathway mapping fundamental ${field} competencies generated by the Sandbox engine.`,
        duration: timeline,
        initialNodes: [
          { id: "node-1", type: "default", position: { x: 250, y: 50 }, data: { title: "Foundations", description: "Understand basic syntax and environment deployment.", link: "https://sandbox.test" } },
          { id: "node-2", type: "default", position: { x: 250, y: 150 }, data: { title: "Architecture", description: "Master system design principles and modules.", link: "https://sandbox.test" } },
          { id: "node-3", type: "default", position: { x: 100, y: 250 }, data: { title: "Advanced Ops", description: "Implementation of CI/CD pipelines.", link: "https://sandbox.test" } },
          { id: "node-4", type: "default", position: { x: 400, y: 250 }, data: { title: "Scaling", description: "Microservices and cloud load balancing.", link: "https://sandbox.test" } },
          { id: "node-5", type: "default", position: { x: 250, y: 350 }, data: { title: "Production", description: "Monitoring, telemetry, and security protocols.", link: "https://sandbox.test" } }
        ],
        initialEdges: [
